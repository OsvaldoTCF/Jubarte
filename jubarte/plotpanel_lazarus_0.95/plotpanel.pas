...
{
 *
 *  File    :		 PlotPanel.pas (version 0.95)
...
// Y-axis (vertical) lines logaritmic
     ...
     repeat
       ...
       repeat
       inc(i);
       if (log10(fXMax)-log10(fXMin)>4) then inc(i);
       if x>=fXMin then
	 begin
	 ...
	 ACanvas.lineto( Round((log10(X)-Log10(fXMin))*Xqnt)+FMarginLeft, height-fMarginBottom);
       ...
       X:=f*i;
       if (log10(fXMax)-log10(fXMin)<0.64) then x:=f*(1+i/2);
       if (log10(fXMax)-log10(fXMin)<0.45) then x:=f*(1+i/5);
       if (log10(fXMax)-log10(fXMin)<0.302) then x:=f*(1+i/10); // oktaaf
       if (log10(fXMax)-log10(fXMin)<0.15) then x:=f*(1+i/20);
     //  if (log10(fXMax)-log10(fXMin)>4) then i:=i+2 else inc(i);
       until (x>=10*f) or (X>fXMax);
       f:=f*10;
       x:=f;
     until (X>fXMax);
end;
...
begin
   ...
   repeat
     ...
     if (x>=fXMin) then
	  ACanvas.TextOut((-ACanvas.TextWidth(Xstr) div 2)+ Round((log10(X)-(log10(fXMin)))*Xqnt)+FMarginLeft ,
     ...
     repeat
      ...
      // x:=f*i;
       ...
       if (x>=fXMin)and (x<fXMax)and(NewPos-LastPos>2.6*ACanvas.TextWidth(Xstr))and (not odd(i)) then
	  begin
     ...
     until (x>=10*f) or (log10(X)>=log10(fXMax));
     f:=f*10;
     x:=f;
     until log10(X)>=log10(fXMax);
end;
